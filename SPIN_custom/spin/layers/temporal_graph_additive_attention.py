...
class TemporalGraphAdditiveAttention(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        if self.root_weight:
            self.lin_skip = Linear(self.tgt_size, self.output_size, bias_initializer="zeros")
        else:
    ...
    def forward(self, x: OptPairTensor, edge_index: Adj, edge_weight: OptTensor = None, mask: OptTensor = None):
        ...
        if self.self_attention is not None:
            ...
            temp = self.self_attention(
                x=(x_src, x_tgt), mask=mask if self.mask_temporal else None, temporal_mask=attn_mask
            )
            out = out + temp
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_weight: OptTensor, mask_j: OptTensor) -> Tensor:
