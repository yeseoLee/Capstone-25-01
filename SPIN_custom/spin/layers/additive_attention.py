...
class AdditiveAttention(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        self.lin_src = Linear(self.src_size, self.output_size, weight_initializer="glorot", bias_initializer="zeros")
...
class TemporalAdditiveAttention(AdditiveAttention):
    ...
    def forward(
        self, x: PairTensor, mask: OptTensor = None, temporal_mask: OptTensor = None, causal_lag: Optional[int] = None
    ):
        ...
        if temporal_mask is None and isinstance(causal_lag, int):
            temporal_mask = tuple(torch.tril_indices(L, S, offset=-causal_lag, device=x_src.device))
