...
def k_hop_subgraph_sampler(
    batch: Batch, k: int, num_nodes: int, max_edges: Optional[int] = None, cut_edges_uniformly: bool = False
):
    N = batch.x.size(-2)
    roots = choice(np.arange(N), num_nodes, replace=False).tolist()
    subgraph = k_hop_subgraph(roots, k, batch.edge_index, relabel_nodes=True, num_nodes=N, flow="target_to_source")
