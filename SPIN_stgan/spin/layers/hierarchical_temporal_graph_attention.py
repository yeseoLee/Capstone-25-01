...
class HierarchicalTemporalGraphAttention(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        if aggr == "softmax":
            aggr = "add"
        super(HierarchicalTemporalGraphAttention, self).__init__(node_dim=-2, aggr=aggr, **kwargs)
    ...
    def forward(self, h: Tensor, z: Tensor, edge_index: Adj, mask: OptTensor = None):
        ...
        h_out = self.propagate(
            h=h_self, z=z_out, edge_index=edge_index, mask=mask if self.mask_spatial else None, size=(n_src, n_tgt)
        )
    ...
    def h_cross_message(self, h_i: Tensor, z_j: Tensor, index, size_i) -> Tensor:
        ...
        if self.spatial_aggr == "softmax":
            ...
            alpha_h = sparse_softmax(alpha_h, index, num_nodes=size_i, dim=self.node_dim)
            h_cross = alpha_h * h_cross
        return h_cross

    def hz_cross_message(self, h_i: Tensor, h_j: Tensor, z_i: Tensor, index, size_i, mask_j: OptTensor) -> Tensor:
        # [batch, steps, edges, channels]
        ...
        if self.spatial_aggr == "softmax":
            ...
            alpha_z = sparse_softmax(alpha_z, index, num_nodes=size_i, dim=self.node_dim)
    ...
    def message(self, h_i: Tensor, h_j: Tensor, z_i: Tensor, z_j: Tensor, index, size_i, mask_j: OptTensor) -> Tensor:
        if self.zh_cross is not None:
